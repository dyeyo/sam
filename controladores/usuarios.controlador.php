...
class ControladorUsuarios{
			...
			if(preg_match('/^[a-zA-Z0-9]+$/', $_POST["ingUsuario"])){
				...
				if($respuesta && password_verify($_POST["ingPassword"], $respuesta["password"])){
					...
					if($respuesta["estado"] == 1){
						...
						if($ultimoLogin == "ok" && $respuesta["primer_acceso"] == 0){
							echo '<script>window.location = "primer-acceso";</script>';
						}
						if($ultimoLogin == "ok" && $respuesta["primer_acceso"] == 1){
    ...
    if(isset($_POST["nuevoUsuario"])) {
        ...
        if(preg_match('/^[a-zA-Z0-9]+$/', $_POST["nuevoUsuario"]) &&
           ...
           preg_match('/^[()\-0-9 ]+$/', $_POST["nuevoPerfil"]) &&
           ...
           preg_match('/^[a-zA-Z0-9]+$/', $_POST["nuevoPassword"])) {
            ...
            if(isset($_FILES["nuevaFoto"]["tmp_name"]) && !empty($_FILES["nuevaFoto"]["tmp_name"])){
                ...
                /* Generar imagen de acuerdo a su tipo */
                $aleatorio = mt_rand(100,999);
                $ruta = "vistas/img/usuarios/".$_POST["nuevoUsuario"]."/".$aleatorio;
            ...
            swal({
                type: "error",
                title: "¡El usuario no cumple con las validaciones!",
                showConfirmButton: true,
                confirmButtonText: "Cerrar"
            }).then(function(result){
                if(result.value){
	...
	static public function ctrChangePass() {
		if ($_SERVER["REQUEST_METHOD"] == "POST") {
			if (isset($_POST["email"]) && isset($_POST["ingPassword"]) && isset($_POST["newPassword"])) {
				$email = $_POST["email"];
				$passwordActual = $_POST["ingPassword"];
				$passwordNueva = $_POST["newPassword"];

				// Consultar la base de datos para obtener la contraseña actual
				$usuario = ModeloUsuarios::mdlObtenerUsuarioPorEmial($email);
				...
				if ($usuario && password_verify($passwordActual, $usuario["password"])) {
					...
					$actualizado = ModeloUsuarios::mdlActualizarPassword($email, $passwordHash);
